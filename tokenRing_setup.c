...
setupSystem()
{
	 ...
	 */
	 ...
	 /*
	 inside node logic, check if terminate is 1 then we need to terminate*/
	for (i = 0; i < N_NODES; i++) {
		...
		/* token_flag = 0 means theres nothing to send, token_flag = 0 means theres something to send*/
	...
	int numberOfPackets;
{
	...
	for (i = 0; i < N_NODES; i++) {
		...
		if (fork() == 0) {
			// i am child
			// call the simulate
			// all the nodes "do their stuff" until they're done
	...
	for (i = 0; i < numberOfPackets; i++) {
		...
		memset(&control->shared_ptr->node[num].to_send.data, '$', control->shared_ptr->node[num].to_send.length);
